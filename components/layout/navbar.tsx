...
export function Navbar({ locale, dictionary }: { locale: Locale; dictionary: NavDictionary }) {
  ...
  const links = dictionary.items.map(([label, id]) => (
    <Link
      key={id}
      href={`/${locale}#${id}`}
      className="rounded-md px-[10px] py-1.5 text-sm font-normal text-muted-foreground transition-colors duration-150 hover:bg-muted hover:text-foreground"
  ...
  return (
    <header className="sticky top-0 z-50 border-b bg-background">
      <nav className="mx-auto flex min-h-14 max-w-[1100px] items-center justify-between px-6">
        <Link href={`/${locale}`} className="flex items-center gap-3 font-medium">
          <span className="flex h-11 w-11 items-center justify-center rounded-full bg-[var(--color-background-info)] text-sm font-medium text-[var(--color-text-info)]">
            MG
          </span>
